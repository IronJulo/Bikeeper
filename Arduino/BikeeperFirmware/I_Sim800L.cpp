...
void I_Sim800L::begin(unsigned int baudrate)
{
	m_softwareSerial->begin(baudrate);
}

void I_Sim800L::listen()
{
	m_softwareSerial->listen();
}

void I_Sim800L::init(char userPhoneNumber[], const char serverPhoneNumber[])
{
	m_stringBuffer->clear();
	listen();
...
bool I_Sim800L::smartRead(const char waiting[], short waitingSize, unsigned int timeout)
{
	bool res = false;
	m_stringBuffer->clear();
...
void I_Sim800L::initTreatement(char userPhoneNumber[], const char serverPhoneNumber[])
{
	char senderPhoneNumber[13];
	short senderIndex = m_stringBuffer->indexOf("+33", 3);
	m_stringBuffer->substring(senderPhoneNumber, senderIndex, senderIndex + 12);

	if (arraysCompare(senderPhoneNumber, serverPhoneNumber))
	{
		if (senderIndex != -1)
		{ //TODO 12 -> 13
...
void I_Sim800L::setModeAT()
{
	m_softwareSerial->println("AT");
}

void I_Sim800L::deleteALL()
{
	m_softwareSerial->println("AT+CMGD=,4");
}

void I_Sim800L::deleteALLRead()
{
	m_softwareSerial->println("AT+CMGD=1,2");
}

void I_Sim800L::setModeTexte()
{
	m_softwareSerial->println("AT+CMGR=1");
}

void I_Sim800L::carriageReturn()
{
	m_softwareSerial->println("\r");
}
